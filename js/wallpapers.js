...
function renderGrid(filter) {
    ...
    items.forEach(function(wp) {
        ...
        if (wp.format === 'portrait') card.classList.add('is-portrait');
        if (wp.format === 'panorama') card.classList.add('is-panorama');
        ...
        card.innerHTML =
            ...
            '<div class="wp-card-overlay">' +
                ...
                '<span class="wp-card-dl"><i class="fas fa-expand"></i> Download</span>' +
            '</div>';
...
function openOverlay(wp) {
    ...
    document.getElementById('wp-overlay-quality').textContent = wp.resolution + '  ·  ' + wp.dimensions + '  ·  ' + wp.format.charAt(0).toUpperCase() + wp.format.slice(1);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('wp-overlay').addEventListener('click', function(e){ if(e.target===this) closeOverlay(); });
    ...
    var form = document.getElementById('wp-lead-form');
    if (form) form.addEventListener('submit', function(e) {
        e.preventDefault();
        document.getElementById('wp-lead-inner').style.display = 'none';
        document.getElementById('wp-lead-success').classList.add('visible');
    });
